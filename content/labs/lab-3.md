...
28. **Реалізувати OAuth (Google login)**
    - У репозиторії поки що немає реалізації OAuth/Google login.
    - Коду `GoogleStrategy`, `passport-google-oauth20` або подібних файлів в `backend/` не знайдено.
...
- Захищені маршрути реалізовано через NestJS guards та JWT стратегію.
- Ролі забезпечують доступ до адміністративного маршруту `GET /auth/admin/users`.
- Валідація введених даних відбувається на рівні DTO за допомогою `class-validator`.
- Використання бази даних забезпечено через Sequelize та модель `User`.
- `Task 5` та `Task 6` є ручними кроками тестування й аналізу, тож у репозиторії містяться лише API-ендпоінти та логування, які дозволяють їх виконувати.
