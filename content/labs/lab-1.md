...
**Місце розташування:**
- GitHub: ([встав посилання](https://github.com/3POSENJOYER/IA-34_appRECORD-YereskoIllia-FIOT-2026-Public))
...
FR-1: Агрегація ігрових даних. Система повинна підтримувати відображення ігор з декількох джерел (Steam, Epic Games Store, GOG) в єдиному уніфікованому інтерфейсі.

FR-2: Аналіз сумісності. Автоматичне зіставлення технічних характеристик гри (мінімальні/рекомендовані вимоги) із поточним апаратним забезпеченням користувача (CPU, GPU, RAM).

FR-3: Прогнозування продуктивності. Розрахунок та візуалізація очікуваної частоти кадрів (FPS) для кожної гри на основі алгоритму порівняння потужності заліза.

FR-4: Система фільтрації та пошуку. Можливість миттєвого пошуку гри за назвою та фільтрація списку за платформою походження або рівнем продуктивності (наприклад, "тільки ігри з 60+ FPS").

FR-5: Візуалізація метрик. Відображення прогрес-барів "здоров'я" сумісності, де колір змінюється від критичного (червоний) до оптимального (зелений).

FR-6: Деталізація заліза. Вивід короткої зведення про поточну систему користувача (наприклад, "Detected: RTX 4070 | Ryzen 7") на головну панель.

Нефункціональні вимоги (Non-Functional Requirements)
NFR-1: Дизайн та естетика (Gaming UI). Реалізація сучасної темної теми з використанням акцентних неонових кольорів (Neon Purple, Accent Green) для створення атмосфери геймерського терміналу.

NFR-2: Адаптивність (Responsiveness). Коректне відображення інтерфейсу на пристроях з різною діагоналлю:

Desktop (1200px+): Багатоколонкова сітка (4-5 карток у ряд).

Tablet (768px - 1024px): Перебудова сітки у 2-3 колонки.

Mobile (менше 768px): Одноколонковий список, трансформація навігації у компактне "бургер-меню".

NFR-3: Продуктивність (Performance). Час ініціалізації додатку та відтворення списку ігор не повинен перевищувати 1.0 сек завдяки використанню Vue 3 Composition API та Vite.

NFR-4: Зручність використання (UX). Використання плавних CSS-анімацій (transition, hover effects) для зворотного зв'язку при взаємодії з картками ігор.

NFR-5: Масштабованість. Архітектура компонентів має дозволяти легко додавати нові ігрові платформи (наприклад, Ubisoft Connect або Battle.net) без переписування логіки відображення.

NFR-6: Доступність. Використання семантичних HTML-тегів (<article>, <nav>, <header>) та відносних одиниць вимірювання (rem, em, %) для гнучкості масштабування тексту.
...
### ER-діаграма
![Скрін 1](../../static/assets/labs/lab-1/Er.jpg)
